(function() {
  ...
  function EnemyWave(params) {
    ...
    function Lib() {
      var lib = {
        1: "good dairy go pen corner bachelor ancient daylight apology decoy degree fix argument atomic demand design bagel double balance bandit basil beastly blast blind for box bobcat bloom bobcat caboose cement canyon circus comet js def moz end dreamy eagle elbow".split(" "),
        2: "elope null energy equal factor escape push edge falcon pull tidy head body foot haml feedback jade true fault color top span left right save fork flex none bold auto finger link size ease fill path fishing hsla from skew sort font size team fishing hash float less float text data".split(" "),
        3: "flour ember event width height clone follow concat fetch valid aside style forward freeze friend galaxy start assign posts logos gallon blogs block garden gather slack class general giant origin comma scrum stroke scale false rails".split(" "),
        4: "inline method deploy target assign window grunt commit minify jekyll stylus article import tweets glance opacity weight bottom scroll italic profile hidden goats keytrap editor grammar grass number growth decimal period habit".split(" "),
        5: "inherit function includes bourbon normalize hammer explorer section ternary heart overflow absolute heavy invalid viewbox content helmet session display background compile bracket backbone boolean heroic dreamhire contains standup".split(" "),
        6: "chriscoyier hobby homeland customers honeycomb hyperbola visibility headphones underscore immediate impossible individual infection customer settings instructor attribute parenthesis markdown compiler responsive jawbreaker keyboard laboratory development".split(" ")
      };
      return lib;
    }
